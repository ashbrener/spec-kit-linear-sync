...
- [ ] T013 [US1] Route the existing `src/reconcile.sh` projection (the 001 repo-Project / spec-Issue / phase-sub-issue / task-checklist path) through the `config::resolved_*` accessors for the default-aliased mapping, with no behaviour change (FR-001, FR-014)
- [ ] T014 [US1] Confirm `src/reconcile.sh` loads + aliases + validates the `mapping:` block before the write loop; default path unchanged from the shipped baseline (regression anchor)
