...
- **Commands:** `/speckit.linear-sync.install`, `/speckit.linear-sync.seed`, `/speckit.linear-sync.push`, `/speckit.linear-sync.pull`, `/speckit.linear-sync.status` (auto-invoked on every `/speckit.*` lifecycle command)
...
graph TD
    ...
    subgraph Bridge["🔁 Reconciler"]
        Push["speckit.linear-sync.push<br/>(read disk → push)"]
    end
    subgraph LN["📋 Linear"]
...
1. The extension installs an `after_*` hook for every `/speckit.*` lifecycle command. Every time you specify, clarify, plan, tasks, implement, or analyze, the hook fires `/speckit.linear-sync.push`.
...
```bash
specify extension add linear-sync --from https://github.com/ashbrener/spec-kit-linear-sync/archive/refs/heads/main.zip
```
...
```bash
specify extension add linear-sync
```

Either path registers the `after_*` hooks into your project's `.specify/extensions.yml`, drops local git hooks into `.git/hooks/`, and scaffolds `.specify/extensions/linear-sync/linear-config.yml`.

If the install reports a vendored `.git/` warning under `.specify/extensions/linear-sync/`, run `rm -rf .specify/extensions/linear-sync/.git` and re-run the install (FR-049). The bridge does not auto-delete that directory — operator consent is required.

## Adopt — the 3 steps every project takes

1. **Run `/speckit.linear-sync.install`.** This is the interactive install ceremony. It resolves your Linear Team UUID (auto-picks if your workspace has one team; prompts otherwise), creates or attaches a Linear Project for this repo, captures your operator identity via Linear's `viewer` query, writes everything to `.specify/extensions/linear-sync/linear-config.yml` (committed to the repo so collaborators inherit the binding), and optionally installs the GitHub Action layer.

2. **Run `/speckit.linear-sync.seed`.** One-shot per Linear workspace — creates the 9 lifecycle workflow states (`Specifying`, `Clarifying`, `Planning`, `Tasking`, `Red-team`, `Implementing`, `Analyzing`, `Ready-to-merge`, `Merged`), the 9 `phase:*` labels, and `task-phase:1`..`task-phase:9`. Captures every UUID at creation and writes it back into `linear-config.yml` (per FR-032) so renames in Linear's UI never break the bridge.

3. **Use spec-kit as normal.** Every `/speckit.specify`, `.clarify`, `.plan`, `.tasks`, `.implement`, `.analyze` now fires `/speckit.linear-sync.push` automatically. The first run for any pre-existing spec backfills retroactively — Linear converges to the correct end-state without producing spurious intermediate-phase comments (FR-014).
...
|---|---|---|
| `/speckit.linear-sync.install` | install-time only | Resolves Team / Project / operator UUIDs, writes `linear-config.yml`, registers hooks, optionally installs the GitHub Action. |
| `/speckit.linear-sync.seed` | workspace setup | Creates lifecycle workflow states + labels. Idempotent; safe to re-run. |
| `/speckit.linear-sync.push` | disk → Linear (write) | The reconciler. Fires automatically on every `/speckit.*` hook; also invokable on demand. |
| `/speckit.linear-sync.pull` | Linear → terminal (read-only) | Cross-repo unified inspect. Surfaces every spec Issue across every repo bound to the workspace. |
| `/speckit.linear-sync.status` | terminal (read-only) | Drift report — per spec, flags mismatches between disk and Linear (lifecycle, branch, last-touched, checklist completion). |
...
`linear-config.yml` is created by `/speckit.linear-sync.install` and committed to your repo. Schema (full spec at [`specs/001-spec-kit-linear-bridge/contracts/config-schema.json`](./specs/001-spec-kit-linear-bridge/contracts/config-schema.json)):
...
| `linear.workflow_state_uuids` | Yes | Map of lifecycle-phase identifier → state UUID. Populated by `/speckit.linear-sync.seed`. Renames in Linear's UI are safe (FR-032). |
| `linear.default_state_uuids` | Yes | UUIDs of the three sub-issue states (`todo` / `in_progress` / `done`). Auto-resolved at seed time. |

`.env` (gitignored) holds `LINEAR_API_KEY` for the local reconcile path. The shipped GitHub Action uses `LINEAR_API_TOKEN` set as a repo secret per `/speckit.linear-sync.install`'s instructions.

## Troubleshooting

### `ERROR: linear-config.yml missing — run /speckit.linear-sync.install first`

The repo hasn't been bound to a Linear workspace yet. Run `/speckit.linear-sync.install` to scaffold the config.

### `ERROR: workspace not seeded — run /speckit.linear-sync.seed`
...
The Action only fires when installed in the repo's default branch. After `/speckit.linear-sync.install` lands the workflow file, merge that change to `main` before opening the PR you want tracked. Token must be set: `gh secret set LINEAR_API_TOKEN`.
